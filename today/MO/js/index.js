...
function weather(data){ //현재 위치에 따른 날씨 정보
		...
		img.setAttribute("src",`../PC/images/icons/${data.weather[0].icon}.gif`);
...
function today(){ //오늘 날짜
	...
	for(let i=0; i<monArr.length; i++){ 
		month.textContent = monArr[m + 1];
	}

	for(let k=0; k<dayWeekArr.length; k++){ 
		dayWeek.textContent = dayWeekArr[dw];
	}
	year.textContent = y;
	date.textContent = d;
}
...
function handMove(){ //손 움직이기
    ...
    function handClick(e){ //손 이미지 변경
        e.preventDefault();
				staticHand.style.visibility = "hidden";
    ...
    function fortuneCont(e){ //내용 보여주기
			moveHand.style.left = e.clientX - 30 + "px";
			moveHand.style.top = e.clientY - 30 + "px";
    ...
    function reset(){ //처음으로
			resetBtn.addEventListener("click",(e)=>{ 
				...
				staticHand.style.visibility = "visible";
				randomCont();
			})
    }
		reset(); 
...
function toDoList(){ //내용 입력
    ...
    function list(){ //할일 목록
        ...
        img.setAttribute("src","../PC/images/remove-before.png");
        ...
        function removeBtn(){ //삭제 버튼
            ...
            remove.addEventListener("mouseover",()=> { 
                img.setAttribute("src","../PC/images/remove-after.png");
            })
...
function selectBox(){ //선택박스
    ...
    const selectList = document.querySelector(".select-list");
		...
		function arrowChange(){ //화살표 이미지 변경
			selectBtn.addEventListener("mouseover",()=>{ 
				selectArrow.style.backgroundImage = "url('../PC/images/arrow-after.png')";
			})
	
			selectBtn.addEventListener("mouseout",()=>{ 
				selectArrow.style.backgroundImage = "url('../PC/images/arrow-before.png')";
			})
		}
		arrowChange();
    ...
    function createEl(e){ //내용 만들기
        ...
        label.textContent = "kcal";
				...
				removeImg.setAttribute("src","../PC/images/remove.png");
